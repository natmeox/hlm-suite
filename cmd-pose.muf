...
: do-pose  ( str -- )
    ...
    who @ dup me @ str_program array_get_ignorers meowed @ array_union  ( str strSpace strName arrWho arrOption )
    ...
    dup if  ( str strSpace strName arrOption )
        meowed @ swap array_diff  ( str strSpace strName arrOption' )
...
: do-nickname pop pop  ( -- )
    ...
    "You will now pose with your %N nickname, if the first three letters match those of your actual character name, no one has your %N for a real name, and your %N contains no spaces." .tellgood  (  )
;
: do-unnickname pop pop  ( strY strZ -- )
    me @ prop_nickname remove_prop
    "You will now pose with your actual character name." .tellgood  (  )
;

: do-nicks pop pop  ( -- )
    me @ prop_nicks remove_prop  (  )
    "You will see poses with nicknames of players who have enabled that feature." .tellgood  (  )
;
: do-unnicks pop pop  ( strY strZ -- )
    me @ prop_nicks "no" setprop
    "You will see all poses with players' actual character names." .tellgood  (  )
;

: do-grey  ( strY strZ -- )
    pop pop  (  )
    me @ prop_color "no" setprop
    me @ "You will not see color in others' poses. Note you will see color in your own poses." fmtstring .tellgood
;
: do-ungrey  ( strY strZ -- )
    pop pop  (  )
    me @ prop_color remove_prop
    "You will see color in poses." .tellgood
;
...
: main  ( str -- )
    ...
    3 pick string? if 3 pick "#" stringpfx if  ( str strX strY strZ )
        ...
        do-pose exit  (  )
    then then
    3 pick int? if pop pop pop do-pose exit then
...
Emotes <message> to the room, like /me on IRC. That is, if Natasha typed "pose waves!", she and everyone else in the room would see "Natasha waves!" Use #ignore <name> to not see <name>'s says, poses, and spoofs. Use #grey to not see color in others' says and poses.

Use #nickname to enable nickname posing, the use of your %N name instead of your character's actual name. You can set your %N nickname with "@set me=%N:<nickname>". The first three letters of your %N must match those of your real name, and must not be another player's name; that is, White-Delirium can be "White" but not "Delimink" and not "Whistlewind" (assuming that name is taken by another player).
